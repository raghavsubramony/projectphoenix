...
Scale up with::
    ...
    py -3 designs/phoenix_v3_optimizer.py --trials 1000 --resume --workers 8
...
def _bounds_for(actuator_mode: bool) -> tuple[ParamBounds, ...]:
    return ACTUATOR_SEARCH_BOUNDS if actuator_mode else SEARCH_BOUNDS
...
class TuningVector:
    ...
    @staticmethod
    def random(rng: random.Random) -> TuningVector:
        return TuningVector(tuple(rng.random() for _ in SEARCH_BOUNDS))
...
def evaluate_vector(
    ...
    *,
    ...
    bounds = _bounds_for(actuator_mode)
...
def _worker_payload(
    values: tuple[float, ...],
    cycles: int,
    base_kwargs: dict[str, float] | None,
    actuator_mode: bool,
) -> TrialResult:
    """Process-pool entry point (must be module-level for Windows spawn)."""
    base = PhoenixV3Config(**base_kwargs) if base_kwargs else None
    return evaluate_vector(
        TuningVector(values), base=base, cycles=cycles, actuator_mode=actuator_mode,
    )
...
def evaluate_batch(
    ...
    *,
    ...
    if workers <= 1:
        return [
            evaluate_vector(v, base=base, cycles=cycles, actuator_mode=actuator_mode)
            for v in vectors
        ]

    base_kw = _config_to_kwargs(base)
    results: list[TrialResult | None] = [None] * len(vectors)
    with ProcessPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(_worker_payload, v.values, cycles, base_kw, actuator_mode): i
...
def trial_to_row(t: TrialResult) -> dict[str, str]:
    row = {k: f"{v:.6f}" if isinstance(v, float) else str(v) for k, v in t.vector.to_dict().items()}
    row.update({
        "score": f"{t.score:.6f}",
...
def row_to_trial(row: dict[str, str]) -> TrialResult:
    params: dict[str, float] = {}
    for name in PARAM_NAMES:
        if name in row and row[name] != "":
            params[name] = float(row[name])
        else:
            b = next(x for x in SEARCH_BOUNDS if x.name == name)
            params[name] = 0.5 * (b.lo + b.hi)
    vector = TuningVector.from_dict(params)
    return TrialResult(
        vector=vector,
...
def load_corpus(path: Path) -> list[TrialResult]:
    if not path.exists():
        return []
    ...
    with path.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            try:
                trials.append(row_to_trial(row))
            except (KeyError, TypeError, ValueError):
                continue
    return trials


def append_corpus(trials: list[TrialResult], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    if not path.exists() or path.stat().st_size == 0:
        write_corpus(trials, path)
        return
    with path.open(newline="", encoding="utf-8") as f:
    ...
    if missing:
        ...
        write_corpus(prior + trials, path)
        return
    with path.open("a", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CORPUS_FIELDS)
        for t in trials:
            writer.writerow(trial_to_row(t))


def write_corpus(trials: list[TrialResult], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def run_optimization(
    *,
    ...
    prior = load_corpus(out_path) if resume else []
    ...
    def run_batch(vectors: list[TuningVector], label: str) -> None:
        ...
        batch = evaluate_batch(
            vectors, base=base, cycles=cycles, workers=workers, actuator_mode=actuator_mode,
        )
        all_trials.extend(batch)
        new_count += len(batch)
        if resume:
            append_corpus(batch, out_path)

    target_new = trials
    pop = max(population, 24)

    # Phase 0 — seed from ideal best tuning (clipped to actuator bounds) when starting fresh
    ...
    if not resume:
        write_corpus(all_trials, out_path)
...
def validate_best_for_export(
    ...
    *,
    ...
    bounds = _bounds_for(actuator_mode)
    cfg = vector.to_config(bounds=bounds)
    if actuator_mode:
        cfg = replace(cfg, **ACTUATOR_PLANT_KWARGS)
    ...
    ideal_eff = 0.0
    if actuator_mode:
        ideal_cfg = replace(vector.to_config(bounds=bounds), thermal_model_enabled=True)
        ideal_cfg = apply_generator_cooling(ideal_cfg, "water_jacket")
...
def export_best_config(
    ...
    *,
    ...
    if validate or search_version == "v3" or actuator_mode:
        best, validated_extras = validate_best_for_export(
            best.vector, actuator_mode=actuator_mode,
        )

    param_dict = best.vector.to_dict(bounds if actuator_mode else None)
...
def main() -> None:
    ...
    print(f"Phoenix V3 optimizer {args.search_version}{' +actuators' if args.actuators else ''}: "
    ...
    export_path = export_best_config(
        report,
        path=args.export_json or (BEST_TUNING_ACTUATOR if args.actuators else None),
