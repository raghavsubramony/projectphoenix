...
from atpe_brain.hil import DeterministicEcuStub, HilHarness, synthetic_commands
...
def main() -> int:
    harness = HilHarness(DeterministicEcuStub(latency_s=0.02, scale_lag=0.5))
    cases = [
    ...
    # Nominal path
    fails = 0
    print("ATPE-BRAIN-VV-001 HIL stub")
    for name, cmd in cases:
        r = harness.step(cmd)
        status = "PASS" if r.pass_ else "FAIL"
        print(f"  {name}: {status} latency={r.ack.latency_s * 1e3:.1f}ms "
              f"indices={r.ack.applied_indices} reasons={r.reasons}")
        if not r.pass_:
            fails += 1

    # Watchdog trip must hold last setpoints (AI cannot bypass).
    ...
    # Slow ECU latency exceeds 50 ms budget.
